...
for (let i = 0; i < picArray.length; i++) {
  ...
  // Add article with class name 'card' to target section
  article.classList.add("card");
  target.appendChild(article);
  
  // Add title inside the article
  h2.innerHTML = picArray[i].title;
  article.appendChild(h2)

  // Add figure element inside the article
  article.appendChild(figure);

  // Add image inside the figure, setting its src and alt attributes
  img.src = picArray[i].image.medium;
  img.alt = picArray[i].title;
  figure.appendChild(img);

  // Add figcaption inside the figure
  figcaption.innerHTML = picArray[i].caption;
  figure.appendChild(figcaption);

  // Add description inside the article in a <p> element
  p.innerHTML = picArray[i].description;
  article.appendChild(p);
  
  // Dialog feature for task 11th
